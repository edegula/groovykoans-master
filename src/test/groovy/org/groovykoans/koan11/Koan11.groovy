...
class Koan11 extends GroovyTestCase {
    ...
    void test01_CreateDb() {
        ...
        Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
            ...
            // ------------ START EDITING HERE ----------------------
            def insertSql = 'INSERT INTO Person (firstname, lastname) VALUES (?,?)'          // use a prepared statement
            def params = ['Jack', 'Dawson']                                                  // to avoid getting boomed by sql injection
            print params
    ...
    void test02_MoreOnInsertingData() {
        ...
        Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
            ...
            new FileReader("$prefix/cast.txt").readLines().each {line ->

                def params = line.toString().split(' ')
                def insertSql = "INSERT INTO Person (firstname, lastname) VALUES (${params[0]},${params[1]})"
                if (params)
            ...
            // ------------ START EDITING HERE ----------------------
            def xml = new XmlSlurper().parse(new File("$prefix/cast2.xml"))

            xml.character.each() {
                def params = it.@actor.text().toString().split(' ')
                def insertSql = "INSERT INTO Person (firstname, lastname) VALUES (${params[0]},${params[1]})"
                if (params)
                    db.executeInsert insertSql
            }
            ...
            // ------------ START EDITING HERE ----------------------
            person.add([firstname:'Bilbo', lastname:'Baggins'])
            // ------------ STOP EDITING HERE  ----------------------
