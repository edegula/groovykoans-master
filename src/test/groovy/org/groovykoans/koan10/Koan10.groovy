...
class Koan10 extends GroovyTestCase {
    ...
    void test01_XmlSlurpersReader() {
        ...
        movieCount = movieCatalog.movie.size()
        ...
        def nodeText = { node -> node.text() }     // closure that returns a node text
        def nodeWithThe = {node -> node.text().toLowerCase().contains('the')}  // closure that checks if the node contains 'the'

        moviesWithThe = movieCatalog.movie.'*'.findAll(nodeWithThe).collect(nodeText)
        ...
        // ------------ START EDITING HERE ----------------------
        movieIdsGreaterThan5 = (movieCatalog.movie.findAll{  movie ->
